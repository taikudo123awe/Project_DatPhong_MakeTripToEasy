  ...
  exports.showDashboard = async (req, res) => {
    try {
      ...
      // ==================== LẤY DỮ LIỆU BOOKING THEO TRẠNG THÁI ====================
      const pendingBookings = await Booking.findAll({
        where: { status: "Chờ nhận phòng" },
        include: [
          {
            model: Room,
            where: { providerId },
      ...
      const inUseBookings = await Booking.findAll({
      ...
      res.render("provider/dashboard", {
        ...
        providerInfo: providerDetail.ProviderInfo,
        providerRooms,
        success,
        error,
        // mới thêm
  ...
  exports.showEditProfileForm = async (req, res) => {
    try {
      ...
      // ⭐ THÊM DÒNG NÀY: Lấy providerInfo để sidebar có logo + tên NCC
  ...
  exports.viewProviderInfo = async (req, res) => {
    try {
      ...
      res.render("provider/view-provider-info", {
        providerInfo,
        provider: req.session.provider, // ⭐ THÊM
        success: success || null,
        active: "profile", // ⭐ ĐỂ SIDEBAR highlight đúng mục
      });
    } catch (err) {
